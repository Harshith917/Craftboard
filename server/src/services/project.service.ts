...
import type { ProjectVisibility } from '../../generated/prisma/enums';
...
export class ProjectsService {
  ...
  async searchProjects(userId: string, query: string) {
    const existingMembershipIds = await prisma.projectMember
      .findMany({ where: { userId }, select: { projectId: true } })
      .then((ms) => ms.map((m) => m.projectId));

    const pendingRequestIds = await prisma.accessRequest
      .findMany({
