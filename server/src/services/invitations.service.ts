...
export class InvitationsService {
  ...
  async inviteByEmail(projectId: string, invitedById: string, email: string, role: string, message?: string, expiresInHours = 48) {
    ...
    if (existingUser?.id) {
      notifyUser(existingUser.id, 'project-invitation', {
  ...
  async inviteByUser(projectId: string, invitedById: string, userId: string, role: string, message?: string, expiresInHours = 48) {
    ...
    notifyUser(userId, 'project-invitation', {
      invitationId: invitation.id,
      projectId: project.id,
      projectName: project.name,
      token,
  ...
  async accept(token: string, currentUserId: string) {
    ...
    notifyUser(invitation.project.ownerId, 'invitation-accepted', {
      projectId: invitation.projectId,
  ...
  async decline(token: string, currentUserId: string) {
    ...
    notifyUser(invitation.project.ownerId, 'invitation-declined', {
      projectId: invitation.projectId,
      projectName: invitation.project.name,
      userId: currentUserId,
    });
  ...
  async resend(invitationId: string, userId: string) {
    ...
    if (invitation.userId) {
      notifyUser(invitation.userId, 'project-invitation', {
        invitationId: invitation.id,
        projectId: invitation.project.id,
        projectName: invitation.project.name,
    ...
    return prisma.projectInvitation.findMany({
      where: {
        OR: [
          ...
          { email: (await prisma.user.findUnique({ where: { id: userId }, select: { email: true } }))?.email ?? '', status: 'pending', expiresAt: { gt: new Date() } },
        ],
      },
      include: {
